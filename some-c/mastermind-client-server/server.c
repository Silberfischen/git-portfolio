...
static uint8_t *read_from_client(int fd, uint8_t *buffer, size_t n)
{
    ...
    do {
        ssize_t r;
        r = recv(connfd, buffer + bytes_recv, n - bytes_recv, 0);
        if (r <= 0) {
    ...
    if (bytes_recv < n) {
    	printf("Sized don't match receive");
    	return NULL;
    }
    return buffer;
}
...
int main(int argc, char *argv[])
{
    ...
    */
	...
	//Variables used for socket init
	int status;
	...
	sprintf(port, "%ld", options.portno);
	...
	if(setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof optval) < 0){
		bail_out(EXIT_FAILURE, "Failed to set sockopt\n");
	}
	...
	if(listen(sockfd, 5) < 0){
		bail_out(EXIT_FAILURE, "Socket listening failed!\n");
	}

	addr_size = sizeof their_addr;
	if((connfd = accept(sockfd, (struct sockaddr *)&their_addr, &addr_size)) < 0){
		bail_out(EXIT_FAILURE, "Unable to accept anything!\n");
	} 
    ...
    for (round = 1; round <= MAX_TRIES && !quit; ++round) {
        ...
        if(send(connfd, buffer, 1, 0) != 1){
        	perror("Could not send answer!\n");
        	bail_out(EXIT_SUCCESS,"Answer not sendable\n");
        }
